...
class FGLNetworkPrinter(ip: String, port: Int, dpi: Int, val diffRendering: Boolean = true) : SocketNetworkPrinter(ip, port, dpi) {
    ...
    override fun printPDF(file: File, numPages: Int) {
        ...
        val bufferedBytes = mutableListOf<Int>()
        try {
            for (f in pages) {
                val p = f.get()
                bufferedBytes.add(p.size)
                ostream.write(p)
                ostream.flush()
                if (bufferedBytes.sum() > 64000) {
                    ...
                    wait@ while (true) {
                        ...
                        when (r) {
                            ...
                            6 -> { bufferedBytes.removeAt(0); break@wait } // ticket ack
